...
import llmlib.architectures.models.bert.hf_bert as hf_bert_module
import llmlib.architectures.models.bert.mosaic_bert as mosaic_bert_module
import llmlib.data.text.mosaic_bert_text_data as text_data_module
import llmlib.data.text.hf_datasets as hf_data    
...
def build_dataloader(cfg, tokenizer, device_batch_size):
    if cfg.name == 'text':
        return text_data_module.build_text_dataloader(cfg, tokenizer,
                                                      device_batch_size)
    elif cfg.name == "hf_text":
        dataset = hf_data.build_hf_dataset(dataset_name=cfg.dataset,
                            data_subset=cfg.data_subset,
                            split=cfg.hf_split,
                            mode=cfg.mode,
                            max_length=cfg.concat_tokens,
                            bos_text=cfg.bos_text,
                            eos_text=cfg.eos_text,
                            no_wrap=cfg.no_wrap,
                            tokenizer=tokenizer)
        loader = hf_data.build_dataloader(dataset=dataset, batch_size=device_batch_size)
        return loader
        # yield from hf_data.generate_samples(loader,
        #                            truncate_num_samples=cfg.truncate_num_samples)
    else:
...
def build_model(cfg: DictConfig):
    if cfg.name == 'hf_bert':
        ...
        return mosaic_bert_module.create_mosaic_bert_mlm(
            ...
            model_config=cfg.model_config,#cfg.get('model_config', None),
            tokenizer_name=cfg.get('tokenizer_name', None),
            gradient_checkpointing=cfg.get('gradient_checkpointing', None))
    else:
