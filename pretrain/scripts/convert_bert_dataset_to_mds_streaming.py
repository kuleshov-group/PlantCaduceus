...
TRY_TOK_PARALLEL = 'false'
os.environ['TOKENIZERS_PARALLELISM'] = TRY_TOK_PARALLEL
...
dna_constants = DatasetConstants(
    chars_per_sample=200_000,  # Computed over validation set
    chars_per_token=1  # OpenAI estimate
)
dna_constants.splits['train'] = DataSplitConstants(hf_split='train',
                                                 folder_split='train',
                                                 raw_samples=1000,#364868892//500,
                                                 truncated_samples=None)

dna_constants.splits['validation'] = DataSplitConstants(hf_split='validation',
                                                 folder_split='validation',
                                                 raw_samples=1000,#364868892//500,
                                                 truncated_samples=None)
dna_constants.splits['test'] = DataSplitConstants(hf_split='test',
                                                 folder_split='test',
                                                 raw_samples=1000,#364868892//500,
                                                 truncated_samples=None)
...
class NoConcatDataset(IterableDataset):
    ...
    def __init__(self, dataset_name: str, data_subset: Union[str, None],
                 split: str, dataset_key : Optional[str]):
        try: 
            ...
            self.hf_dataset = hf_datasets.load_from_disk(dataset_name)[split].to_iterable_dataset()
    ...
    def __iter__(self) -> Iterable[Dict[str, bytes]]:
        ...
        for sample in self.hf_dataset:
            ...
            yield {'text': str(sample[key]).encode('utf-8')} #} #.encode('utf-8')}


class NoConcatDatasetWeight(IterableDataset):
    """An IterableDataset that returns text samples for MDSWriter.
    ...
    def __iter__(self) -> Iterable[Dict[str, bytes]]:
        ...
        for sample in self.hf_dataset:
            encoded = self.tokenizer(sample[key],
            ...
            #print(sample[key])
            # convert to bytes to store in MDS binary format
            loss_weight = np.ones_like(encoded, dtype=float)
            loss_weight[
                np.char.islower(sample[key])
                #np.char.islower([list(x) for x in sample[key]])
            ] = 0.1
            out = {'tokens': np.asarray(encoded["input_ids"]).tobytes(),
                   'loss_weight' : np.asarray(loss_weight).tobytes(),}
            # for k,v in sample.items():
            #     if k not in out and k != self.dataset_key:
            #         if isinstance(v, str):
            #             out[key] = np.asarray(v)
            #         else:
            #             out[key] = torch.tensor(v)
            #print({k:type(v) for k,v in out.items()})
            
            #out.update({k:v for k,v in sample.items()y})
            yield out
#{'input_ids': str(sample[key]).encode('utf-8')} #} #.encode('utf-8')}
...
class ConcatTokensDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterable[Dict[str, bytes]]:
        ...
        seq_id = "sequence"
        for sample in self.hf_dataset:
            encoded = self.tokenizer(sample[seq_id],
                                     truncation=False,
                                     padding=False)
...
def build_hf_dataset(dataset_name: str,
                     ...
                     data_subset: Union[str, None] = None,
                     dataset_key : Optional[str] = None) -> IterableDataset:
    ...
    if mode == ConcatMode.NO_CONCAT:
        dataset = NoConcatDataset(dataset_name=dataset_name,
                                  ...
                                  split=split,
                                  dataset_key="seq") # sequence
    elif True:
        dataset = NoConcatDatasetWeight(dataset_name=dataset_name,
                                  data_subset=data_subset,
                                  split=split,
                                  dataset_key="seq",
                                  tokenizer=tokenizer,
...
def build_dataloader(dataset, batch_size) -> DataLoader:
    ...
    if 'linux' in platform.platform().lower():
        if  hasattr(dataset.hf_dataset, "n_shards"):
...
def main(args: Namespace) -> None:
    ...
    try:
        ...
        dataset_constants = dna_constants
        #raise ValueError(
