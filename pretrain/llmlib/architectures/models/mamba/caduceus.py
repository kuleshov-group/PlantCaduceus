...
all = ['create_caduceus_mlm']


def create_caduceus_mlm(pretrained_model_name: Optional[str] = None ,
    ...
    if not pretrained_model_name:
        pretrained_model_name = 'yairschiff/caduceus_base'

    assert tokenizer_name is not None, f'{tokenizer_name=}'
    tokenizer = transformers.AutoTokenizer.from_pretrained(tokenizer_name, trust_remote_code=True)#, model_max_length=model_config.max_position_embeddings)
#tokenizer = AutoTokenizer.from_pretrained("gonzalobenegas/tokenizer-dna-mlm")
    #tokenizer.add_special_tokens({"sep_token": "[SEP]"})  # Add SEP token
    complement_map = {"A": "T", "C": "G", "G": "C", "T": "A",
                    "a": "t", "c": "g", "g": "c", "t": "a"}
    complement_map_ids = {}
    for k, v in sorted(tokenizer.get_vocab().items(), key=lambda x: x[1]):
        comp_id = tokenizer.get_vocab()[complement_map[k]] if k in complement_map.keys() else v
        complement_map_ids[tokenizer.get_vocab()[k]] = comp_id
    ...
    if use_pretrained:
        ...
        model = transformers.AutoModelForMaskedLM.from_pretrained(
            pretrained_model_name_or_path=pretrained_model_name, trust_remote_code=True, **model_config)
    else:
        model_config.setdefault("vocab_size", tokenizer.vocab_size)
        model_config.setdefault("pad_token_id", tokenizer.pad_token_id)
        config = transformers.AutoConfig.from_pretrained(pretrained_model_name, complement_map=complement_map_ids, trust_remote_code=True, **model_config)
        ...
        model = transformers.AutoModelForMaskedLM.from_config(config, trust_remote_code=True)
    ...
    # # setup the tokenizer 
    # if tokenizer_name:
    #     tokenizer = transformers.AutoTokenizer.from_pretrained(tokenizer_name, model_max_length=config.max_position_embeddings)
    # else:
    #     tokenizer = transformers.AutoTokenizer.from_pretrained(
    #         pretrained_model_name, model_max_length=config.max_position_embeddings)

    metrics = [
        LanguageCrossEntropy(ignore_index=-100),#tokenizer.pad_token_id), # Metric is screwed up for LossWeighting
                             #vocab_size=model.config.vocab_size),
        MaskedAccuracy(ignore_index=-100)#tokenizer.pad_token_id)
    ]
    ...
    hf_model.model._fsdp_wrap = True # try to improve FSDP wrap
    return hf_model
...
if __name__ == '__main__':
    model_config = {"d_model": 384, "n_layer": 20}
    model = create_caduceus_mlm(tokenizer_name="songlab/tokenizer-dna-mlm", model_config=model_config)
